...
def test_secret_vault_tab_access_and_lock():
    ...
    # 1. Visiting /admin/secret-vault when locked shows password form
    res = client.get("/admin/secret-vault")
    assert res.status_code == 200
    assert b"Protected Secret Section" in res.data
    assert b"Unlock Secret Vault" in res.data
    ...
    # 3. Submitting CORRECT password unlocks secret tab
